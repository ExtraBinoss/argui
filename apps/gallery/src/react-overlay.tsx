...
import { ReactPopover } from './react-controls'
...
/** Shows the same native filter samples through the React adapter. */
export function ReactOverlayPage(props: { theme: Palette }): ReactElement {
  return <column width="fill" gap={16}>
    ...
    <row width="fill" wrap={true} gap={16}>
      {overlayFilters.map((filter) => <column key={filter.name} nativeKey={`overlay-${filter.name}`} gap={7}>
