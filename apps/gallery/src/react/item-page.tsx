...
import { Badge, Button, Item, ItemActions, ItemContent, ItemDescription, ItemFooter, ItemGroup, ItemHeader, ItemMedia, ItemSeparator, ItemTitle, type Palette } from '@argui/widgets/react'

/** Demonstrates item groups, row variants, media, selection, and footer actions. */
export function ItemPage(props: { theme: Palette }): ReactElement {
  ...
  return <column width="fill" gap={16}>
    ...
    <ItemGroup label="Recent work items">
      ...
      <Item id="item-select-release" theme={props.theme} label="Select release notes"
        variant="muted" selected={selected} onClick={() => setSelected((value) => !value)}>
        <ItemMedia theme={props.theme} variant="icon">
