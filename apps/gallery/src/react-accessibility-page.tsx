...
/** Exercises the same authored native semantics through the React adapter. */
export function ReactAccessibilityPage(props: { theme: Palette }): ReactElement {
  ...
  return <column width="fill" gap={16}>
    ...
    <text nativeKey="action-help" text="A Rectangle acting as a button, with a description and keyboard activation."
      color={props.theme.muted} font_size={13} width="fill" />
    <rectangle nativeKey="custom-action" role="button" accessible_name="Run accessible action"
    ...
    <rectangle role="switch" accessible_name="Enable notifications"
      ...
      keyboard_activation="enter_or_space" onClick={() => setChecked((value) => !value)}
    ...
    <text nativeKey="level-label" text="Volume" color={props.theme.foreground} font_size={15} />
    <text nativeKey="level-help" text="Use the screen reader increase, decrease or set value actions."
