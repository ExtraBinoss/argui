...
import { Button, InputField, type Palette } from '@argui/widgets/solid'

/** Exercises native service success, cancellation, and error paths from Solid. */
export function ServicesPage(props: { services: ApplicationServices; theme: Palette }): JSX.Element {
  const [status, setStatus] = createSignal('Choose a native service.')
  const [trayEnabled, setTrayEnabled] = createSignal(true)
  const [closeHides, setCloseHides] = createSignal(true)
  const [preferredCloseHides, setPreferredCloseHides] = createSignal(true)
  const [message, setMessage] = createSignal('Bonjour from the main gallery')
  const [windowTitle, setWindowTitle] = createSignal('Argui Companion')
  const [windowWidth, setWindowWidth] = createSignal('480')
  const [windowHeight, setWindowHeight] = createSignal('260')
  const [targetWindow, setTargetWindow] = createSignal<'main' | 'companion'>('companion')
  const [decorations, setDecorations] = createSignal(true)
  const [transparent, setTransparent] = createSignal(false)
  const [backdrop, setBackdrop] = createSignal(false)
  const [backdropScope, setBackdropScope] = createSignal<'window' | 'panel'>('window')
    ...
    <row wrap={true} gap={8}>
      ...
      <Button id="service-open-companion" label="Open companion window" theme={props.theme}
        onClick={() => run('Companion', (signal) => props.services.call('windows', 'openCompanion', {
          ...
          decorations: decorations(), transparent: transparent(),
          backdrop: backdrop(), backdropScope: backdropScope(),
        }, signal))} />
      <Button id="service-send-companion" label="Send to companion" theme={props.theme}
        onClick={() => run('Send message', (signal) => props.services.call('windows', 'sendMessage', { message: message() }, signal))} />
