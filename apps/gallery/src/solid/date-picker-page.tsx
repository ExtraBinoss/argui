...
export function DatePickerPage(props: { theme: Palette }): JSX.Element {
  const today = new Date()
  const [date, setDate] = createSignal(today)
  const [input, setInput] = createSignal(dateKey(today))
  ...
  return <column width="fill" gap={14}>
    ...
    <row gap={8} align_items="center">
      ...
      <Button id="date-picker-locale" label={locale() === 'en-US' ? 'Use French' : 'Use English'}
        ...
        onClick={() => setLocale(locale() === 'en-US' ? 'fr-FR' : 'en-US')} />
    </row>
