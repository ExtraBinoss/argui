...
animation tick, reset, pause, or simulated error advances an explicit content
revision. The callback copies the snapshot, releases the lock, writes its
uniform buffer, and encodes work. It never submits or presents. While paused,
the model requests no animation frames, the revision stays fixed, and unrelated
Argui redraws reuse the retained texture without invoking the callback.
...
pause/resume. “Simulate error” exercises the visible placeholder and runtime
