...
test('React VirtualList mounts only native-requested rows in a million-item collection', async () => {
  ...
  act(() => root.render(<VirtualList count={1_000_000} itemKey={(index) => `row-${index}`} height={320}
    estimate={40} variable={false} dataVersion={3}
  ...
  const dataVersion = list.type.properties.find((property) => property.name === 'dataVersion')!
  ...
  await act(async () => deliver({ node: list.id, callback: list.listeners.get(event.id)!, payload: {
  ...
  const rowText = list.children[0]?.children[0]
  const textProperty = rowText?.type.properties.find((property) => property.name === 'text')
  expect(rowText?.values.get(textProperty!.id)?.value).toContain('500000')
