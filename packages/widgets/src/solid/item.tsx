...
export function Item(props: ItemProps): JSX.Element {
  ...
  const content = <touchArea enabled={interactive() && !props.disabled} mouse_cursor={props.disabled ? 'not_allowed' : interactive() ? 'pointer' : 'default'}
    ...
    onPointerDown={() => { if (interactive() && !props.disabled) { pointerFocus = true; setPressed(true); setFocused(false) } }}
  ...
  return <focusScope key={props.id ? `${props.id}-item` : undefined} role="list_item"
    accessible_name={interactive() ? undefined : props.label} selected={props.selected}>
    {interactive() ? <focusScope key={props.id} role="button" accessible_name={props.label}
