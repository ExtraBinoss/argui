...
export type ChartDatum = import('../shared/foundation-k').FoundationKChartDatum
export type ChartProps = FoundationKChartProps

/** Renders a selectable, accessible native horizontal bar chart. */
export function Chart(props: ChartProps): JSX.Element {
  const data = () => foundationKChartData(props.data)
  const [uncontrolled, setUncontrolled] = createSignal<string | null>(props.defaultSelectedId ?? null)
  const initialIndex = data().findIndex((datum) => datum.id === (props.selectedId ?? props.defaultSelectedId))
  const [activeIndex, setActiveIndex] = createSignal(Math.max(0, initialIndex))
  ...
  const currentIndex = () => {
    return Math.min(Math.max(0, activeIndex()), Math.max(0, data().length - 1))
  }
  ...
  const onKey = (payload: unknown) => {
    ...
    if (key === 'ArrowDown' || key === 'ArrowRight') next = Math.min(entries.length - 1, next + 1)
  ...
  const renderBars = () => data().map((datum, index) => {
  ...
  return <column width={width} gap={10}>
    ...
    <focusScope key={`${props.id}-list`} role="list_box" accessible_name={props.title}
      ...
      orientation="vertical" focusable={data().length > 0} enabled={data().length > 0}
      onKey={onKey}>
      <rectangle width="fill" height={height} background={props.theme.surface}
        ...
        {data().length > 0
          ? <column width="fill" height="fill" gap={2} padding={10} justify_content="center">{renderBars()}</column>
