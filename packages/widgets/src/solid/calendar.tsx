...
export function Calendar(props: CalendarProps): JSX.Element {
  ...
  return <column width={294} gap={8} role="group" accessible_name={props.label ?? 'Choose date'}>
    ...
    <focusScope key={gridId} role="grid" accessible_name={props.label ?? 'Choose date'}
      ...
      <column width="fill" gap={3}>
        <row width="fill" gap={3} role="row">
          {days.map((day) => <column key={`${props.id}-weekday-${day.full}`} width={38} height={24}
        ...
        {Array.from({ length: 6 }, (_, weekIndex) => <row key={`${props.id}-week-${weekIndex}`} width="fill" gap={3} role="row">
          {monthCells().slice(weekIndex * 7, weekIndex * 7 + 7).map((cell) => {
            if (!cell.inMonth && props.showOutsideDays === false) {
              return <rectangle key={`${props.id}-blank-${cell.key}`} width={38} height={36} accessible_hidden={true} />
            }
