...
export type ScrollAreaProps = SurfaceScrollAreaProps<JSX.Element>

/** Renders a fixed-size viewport that scrolls with native Argui input handling. */
export function ScrollArea(props: ScrollAreaProps): JSX.Element {
  ...
  if (orientation === 'vertical') {
    return <rectangle width={width} height={height} background={props.theme.surface}
      ...
      <focusScope key={`${props.id}-focus`} width="fill" height="fill" focusable={false}
        ...
        <column key={props.id} width="fill" height="fill" padding={padding} gap={8}
  ...
  return <rectangle width={width} height={height} background={props.theme.surface}
    ...
    <focusScope key={`${props.id}-focus`} width="fill" height="fill" focusable={false}
      focus_on_tab_navigation={false} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}>
      <flickable key={props.id} width="fill" height="fill" scroll_x={horizontal} scroll_y={vertical}
