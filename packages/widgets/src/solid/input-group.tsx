...
export function InputGroup(props: InputGroupProps): JSX.Element {
  ...
  return <InputGroupContext.Provider value={{ setFocused }}>
    {content}
...
export function InputGroupButton(props: InputGroupButtonProps): JSX.Element {
  ...
  return <focusScope key={props.id} role="button" accessible_name={props.label} enabled={!props.disabled}
...
export function InputGroupInput(props: InputGroupInputProps): JSX.Element {
  return <InputGroupControl {...props} multiline={false} />
}
...
export function InputGroupTextarea(props: InputGroupTextareaProps): JSX.Element {
  return <InputGroupControl {...props} multiline={true} />
}

interface InputGroupControlInternalProps extends InputGroupControlProps {
  multiline: boolean
  height?: number
}
...
function InputGroupControl(props: InputGroupControlInternalProps): JSX.Element {
  ...
  return <container width="fill" min_width={0} grow={1} height={height}>
    <textInput key={props.id} width="fill" height="fill" clip={true} role={props.multiline ? 'text_area' : 'text_input'}
      ...
      onFocus={() => group?.setFocused(true)}
      onBlur={() => group?.setFocused(false)}
