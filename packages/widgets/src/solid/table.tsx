...
export type TableProps = FoundationKTableProps

/** Renders themed table rows with accessible sorting and optional row selection. */
export function Table(props: TableProps): JSX.Element {
  const [uncontrolledSort, setUncontrolledSort] = createSignal<FoundationKTableSort | null>(props.defaultSort ?? null)
  const [uncontrolledSelection, setUncontrolledSelection] = createSignal<readonly string[]>(props.defaultSelectedRowIds ?? [])
  ...
  const sortBy = (columnId: string) => {
    ...
    const next = current?.columnId !== columnId
      ...
      : current.direction === 'ascending' ? { columnId, direction: 'descending' as const } : null
  ...
  const headerRow = <focusScope key={`${props.id}-header-row`} role="row" accessible_name="Table headers">
  ...
  const bodyRows = rows().length === 0
    ? <row key={`${props.id}-empty`} width="fill" height={52} padding_left={12} align_items="center">
  ...
  return <column width="fill" gap={6}>
    ...
    <focusScope key={`${props.id}-table`} role="table" accessible_name={props.label}
