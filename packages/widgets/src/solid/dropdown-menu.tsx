...
export function DropdownMenu(props: DropdownMenuProps): JSX.Element {
  ...
  const [initialActiveId, setInitialActiveId] = createSignal('')
  ...
  return <column width="fit" gap={0}>
    <focusScope key={triggerId} role="button" accessible_name={props.label ?? props.triggerLabel}
...
export function SurfaceMenuPopup(props: SurfaceMenuPopupProps): JSX.Element {
  const rows = () => surfaceBFlatten(props.items)
  const navigable = () => surfaceBFocusable(props.items)
  const [activeId, setActiveId] = createSignal(props.initialActiveId ?? navigable()[0]?.id ?? '')
  ...
  const onKey = (payload: unknown) => {
    ...
    if (key.length === 1 && key !== ' ') {
      searchBuffer += key.toLocaleLowerCase()
      const match = surfaceBSearch(props.items, active(), searchBuffer)
      if (match) { setActiveId(match); setOpenSubmenu('') }
      if (searchTimer) clearTimeout(searchTimer)
      searchTimer = setTimeout(() => { searchBuffer = '' }, 700)
    }
  }
  const enabledCount = () => navigable().length
  return <popupWindow key={props.id} anchor={props.anchor} placement={props.placement} width={props.width}
    ...
    <focusScope key={`${props.id}-menu`} role="menu" accessible_name={props.label}
      ...
      <rectangle width="fill" background={props.theme.surface} border_color={props.theme.border} border_width={1}
        ...
        <column width="fill" gap={2} padding={4}>
          {rows().map((item) => renderRow(item, props, active(), enabledCount(), activate, setActiveId, setOpenSubmenu,
            openSubmenu() === item.id))}
        </column>
      </rectangle>
    </focusScope>
...
function renderRow(
  ...
  return <column key={key} width="fill" gap={0}>
    <focusScope key={key} role={role} accessible_name={item.label}
    ...
    {item.type === 'submenu' && submenuOpen && !disabled ? <SurfaceMenuPopup id={`${props.id}-sub-${surfaceBKeyPart(item.id)}`}
      anchor={key} placement="right_start" label={item.label} items={item.items} theme={theme}
      width={props.width} controller={props.controller} onClose={() => setOpenSubmenu('')} nested /> : null}
  </column>
}
