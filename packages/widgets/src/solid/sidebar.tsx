...
export function Sidebar(props: SidebarProps): JSX.Element {
  ...
  const [internalExpanded, setInternalExpanded] = createSignal<Record<string, boolean>>({ ...(props.defaultExpandedItems ?? {}) })
  ...
  const renderItem = (item: SurfaceSidebarItem, depth: number): JSX.Element => {
    ...
    return <column key={`${key}-branch`} width="fill" gap={2}>
      <focusScope key={key} role="button" accessible_name={item.label} tooltip={iconOnly() ? item.label : undefined}
      ...
      {isOpen ? <column key={`${key}-children`} width="fill" gap={2} padding_left={6}
  ...
  return <focusScope key={props.id} role="navigation" accessible_name={props.label ?? 'Sidebar'}
    ...
    <rectangle width={panelWidth()} height={props.height ?? 360} background={props.theme.surfaceRaised}
      ...
      <column width="fill" height="fill" gap={6} padding={7} scroll_y={contentVisible()}>
        {mode() !== 'none' ? <focusScope key={toggleId} role="button"
        ...
        {contentVisible() ? <column key={`${props.id}-items`} width="fill" gap={2}>
