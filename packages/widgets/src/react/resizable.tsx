...
export function ReactResizable(props: SurfaceEResizableProps<ReactElement>): ReactElement {
  ...
  const [localValue, setLocalValue] = useState(surfaceEClampSplit(props.defaultValue ?? 50, min, max))
  ...
  const adjust = (delta: number) => commit(valueRef.current + delta)
  ...
  const divider = <focusScope nativeKey={`${props.id}-separator`} role="separator"
  ...
  return orientation === 'horizontal'
    ? <row nativeKey={props.id} width={extent} height={crossExtent} gap={0} role="group" accessible_name={props.label ?? 'Resizable panels'}>
    ...
    : <column nativeKey={props.id} width={crossExtent} height={extent} gap={0} role="group" accessible_name={props.label ?? 'Resizable panels'}>
