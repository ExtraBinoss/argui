...
export function ReactSelect(props: ReactSelectProps): ReactElement {
  ...
  const open = () => {
    ...
    setActiveIndex(selected >= 0 ? selected : nextSelectIndex(options, -1, 1))
    setExpanded(true)
  }
  ...
  const onKey = (payload: unknown) => {
    ...
    if (key === 'Escape') setExpanded(false)
      ...
      const next = nextSelectIndex(options, activeIndex < 0 ? (key === 'ArrowDown' ? -1 : 0) : activeIndex,
        key === 'ArrowDown' ? 1 : -1)
      if (next >= 0) { setActiveIndex(next); setExpanded(true) }
    } else if (key === 'Home' || key === 'End') {
      const next = nextSelectIndex(options, key === 'Home' ? -1 : 0, key === 'Home' ? 1 : -1)
      if (next >= 0) { setActiveIndex(next); setExpanded(true) }
      ...
      const next = options.findIndex((option) => !option.disabled && option.label.toLocaleLowerCase().startsWith(search))
      if (next >= 0) { setActiveIndex(next); setExpanded(true) }
    }
  }
  return <column gap={8}>
    ...
    {expanded ? <popupWindow nativeKey={`${props.id}-popup`} anchor={props.id} placement="bottom_start"
      ...
      <focusScope role="list_box" accessible_name={props.label} focusable={true}
        ...
        <rectangle width={width} background={props.theme.surface}
          ...
          <column width="fill" max_height={280} scroll_y={true} gap={2} padding={4}>
            {options.map((option, index) => <column key={`${props.id}-row-${index}`}
              nativeKey={`${props.id}-row-${index}`} width="fill" gap={2}>
              {option.group && (index === 0 || options[index - 1]?.group !== option.group)
                ? <text text={option.group} color={props.theme.muted} font_size={11} /> : null}
              <focusScope nativeKey={`${props.id}-option-${index}`} role="option"
