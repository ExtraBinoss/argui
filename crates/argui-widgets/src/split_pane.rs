...
impl SplitPane {
    #[must_use]
    pub fn new(key: impl Into<String>, axis: SplitAxis, size: f32, minimum: f32, maximum: f32) -> Self {
        assert!(minimum.is_finite() && maximum.is_finite() && minimum >= 0.0 && maximum >= minimum);
        assert!(size.is_finite());
        let size = size.clamp(minimum, maximum);
        Self { key: key.into(), axis, size, minimum, maximum, reset: size, start: None, trailing: false }
    }
    ...
    pub fn update(&mut self, event: &UiEvent) -> bool {
        if event.target_key() != Some(self.key.as_str()) { return false; }
        let mut value = self.size;
        match &event.kind {
            UiEventKind::Gesture(gesture) => {
                let GestureKind::Pan { total, .. } = gesture.kind else { return false; };
                if gesture.phase == GesturePhase::Started { self.start = Some(self.size); }
                if let Some(start) = self.start {
                    if gesture.phase != GesturePhase::Cancelled {
                        let delta = match self.axis { SplitAxis::Horizontal => total.x, SplitAxis::Vertical => total.y };
            ...
            UiEventKind::KeyInput(input) if input.state == KeyState::Pressed => {
                ...
                value = match (&input.key, self.axis) {
                    (Key::ArrowLeft, SplitAxis::Horizontal) | (Key::ArrowUp, SplitAxis::Vertical) => self.size - step * direction,
                    (Key::ArrowRight, SplitAxis::Horizontal) | (Key::ArrowDown, SplitAxis::Vertical) => self.size + step * direction,
    ...
    pub fn separator(&self, theme: &WidgetTheme) -> Element {
        ...
        let line = Element::container([])
            .width(if horizontal { length(2.0) } else { percent(1.0) })
            .height(if horizontal { percent(1.0) } else { length(2.0) })
            .background(theme.border);
        Element::container([line])
            .keyed(self.key.clone())
            .width(if horizontal { length(6.0) } else { percent(1.0) })
            .height(if horizontal { percent(1.0) } else { length(6.0) })
            .shrink(0.0)
            .align_items(AlignItems::CENTER)
            .justify_content(JustifyContent::CENTER)
            .user_select(UserSelect::None)
            .interaction(Interaction::default().focusable(true)
                .cursor(if horizontal { CursorIcon::EwResize } else { CursorIcon::NsResize })
                .gestures(GestureSet::default().pan(PanGesture::default()
                    .axis(if horizontal { PanAxis::Horizontal } else { PanAxis::Vertical })
                    .immediate().capture(GestureCapture::OnPress).delivery(GestureDelivery::FrameCoalesced))))
            .semantics(Semantics::new(Role::Separator).label("Resize panels")
                .orientation(if horizontal { Orientation::Vertical } else { Orientation::Horizontal })
                .value(SemanticValue::Number { value: self.size as f64, minimum: Some(self.minimum as f64), maximum: Some(self.maximum as f64), step: Some(10.0) })
                .action(SemanticAction::Focus))
            .when(VisualState::Hovered, StylePatch::new().set(property::BackgroundColor, theme.primary))
            .when(VisualState::Pressed, StylePatch::new().set(property::BackgroundColor, theme.primary))
            .when(VisualState::FocusVisible, StylePatch::new().set(property::BackgroundColor, theme.primary))
    }

    #[must_use]
    pub fn build(&self, first: Element, separator: Element, second: Element, available: f32, other_minimum: f32) -> Element {
        ...
        let fixed = |pane: Element| match self.axis {
            ...
            SplitAxis::Vertical => pane.height(length(size)).min_height(length(0.0)).shrink(0.0),
        };
        let flexible = |pane: Element| pane.grow(1.0).min_width(length(0.0)).min_height(length(0.0));
        let children = if self.trailing { [flexible(first), separator, fixed(second)] } else { [fixed(first), separator, flexible(second)] };
        match self.axis {
