...
fn exercise() {
    ...
    if std::env::var_os("ARGUI_POPUP_TEST_CHILD").is_none() {
        ...
        let status = std::process::Command::new(root.join("scripts/linux-hidden-display.sh"))
            ...
            .env("ARGUI_POPUP_TEST_SUCCESS", &success)
            .current_dir(&root)
            .arg("timeout")
            .arg("30s")
        ...
        assert!(
            ...
            "hidden native popup test failed"
        );
        let _ = std::fs::remove_file(success);
        return;
    }
    ...
    if let Some(success) = std::env::var_os("ARGUI_POPUP_TEST_SUCCESS") {
        std::fs::write(success, "passed").unwrap();
    }
}
fn main() {
