    ...
    pub fn build(
        ...
        for command in display_list.commands() {
            match command {
                ...
                DisplayCommand::BeginLayer(style) => stack.push(EffectLayer {
                    style: style.scaled(scale_factor),
                    children: Vec::new(),
                }),
            ...
            for node in nodes {
                match node {
                    ...
                    EffectNode::Layer(layer) => {
                        ...
                        stats.filter_passes += layer.style.filters.len()
                            + layer.style.backdrop_filters.len()
...
mod tests {
    ...
    fn nested_layers_preserve_draw_order_and_report_cost() {
        ...
        let graph = EffectGraph::build(&list, &[0..2, 2..4, 4..7], 1.0).unwrap();
    ...
    fn graph_skips_empty_draws_and_collects_nested_custom_effects() {
        ...
        let graph = EffectGraph::build(&list, &[0..2, 2..4, 0..0], 1.0).unwrap();
        assert_eq!(graph.stats().draw_batches, 1);
        assert_eq!(graph.custom_effects(), vec![&foreground, &backdrop]);

        let plain = EffectGraph::build(&argui_paint::DisplayList::new(), &[], 1.0).unwrap();
