...
impl SurfaceRenderer {
        ...
        T: Into<SurfaceTarget<'static>> + Clone,
    {
        ...
        {
            let webgpu = configure::browser_instance();
            webgpu
                ...
                .await
                .map_err(|error| {
                    RendererError::AdapterRequest(format!(
                        "Argui Web requires a WebGPU adapter; this browser did not provide one: {error}"
                    ))
                })?;
            Self::new_with_instance(target, width, height, renderer_config, webgpu, None).await
        }
