...
/// Identifies one directed endpoint of a document text selection.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DocumentSelectionEndpoint {
    /// The selection's anchor endpoint.
    Anchor,
    /// The selection's focus endpoint.
    Focus,
}
...
struct DocumentSelectionDrag {
    ...
    endpoint: Option<DocumentSelectionEndpoint>,
}
...
impl UiTree {
    ...
    pub fn drag_document_selection(&mut self, point: DocumentTextPoint) -> InteractionUpdate {
        ...
        if let Some(endpoint) = drag.endpoint {
            let Some(mut selection) = self.document_selection.selection else {
                return InteractionUpdate::default();
            };
            match endpoint {
                DocumentSelectionEndpoint::Anchor => selection.anchor = point,
                DocumentSelectionEndpoint::Focus => selection.focus = point,
            }
            return self.set_document_selection(selection, Some(drag));
        }
