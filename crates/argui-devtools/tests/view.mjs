...
try {
    for (const scheme of ['dark', 'light']) {
        ...
        const previewRect = await rect(button('Live button preview'));
        const held = await capture('gallery-drag-held');
        for (const point of [[previewRect.x + 12, previewRect.y + 12], [650, 355]]) {
            const rgb = await pixel(held, ...point);
            assert.ok(rgb.every((v, i) => Math.abs(v - [128, 64, 64][i]) <= 2), `Both previews update while dragging: ${rgb}`);
        }
        ...
        assert.notDeepEqual(await pixel(before, 650, 355), await pixel(hover, 650, 355), 'Application pixels show hovered bounds');
        ...
        await click(button('%'));
        const percent = await rect('input[aria-label="width %"]');
        const unit = await rect(button('%'));
        assert.ok(Math.abs(percent.y - unit.y) <= 2 && percent.width > 40, 'Unit and input stay on one compact row');
        await fill('input[aria-label="width %"]', '100');
        await capture('properties-percent');
        ...
        const toolsPads = await page.$$('[role=group][aria-label^="Saturation "]');
        const toolsPad = await toolsPads.at(-1).evaluate(el => el.getBoundingClientRect().toJSON());
        await page.mouse.move(toolsPad.x + toolsPad.width / 2, toolsPad.y + toolsPad.height / 2);
        await page.mouse.down(); await pause();
        const heldProperty = await capture('property-drag-held');
        assert.ok((await pixel(heldProperty, 650, 355)).every((v, i) => Math.abs(v - [64, 128, 64][i]) < 3), 'Element color updates before pointer release');
        const heldPad = await (await page.$$('[role=group][aria-label^="Saturation "]')).at(-1).evaluate(el => el.getBoundingClientRect().toJSON());
        assert.deepEqual(heldPad, toolsPad, 'Popover does not move during its gesture');
        await page.mouse.up(); await page.keyboard.press('Escape'); await pause();
        assert.equal(await page.$('input[aria-label="background HEX"]'), null, 'Escape closes property popover');
        const swatch = await pixel(green, 650, 355); assert.ok(swatch[1] > 200 && swatch[0] < 100, 'Property override changes app pixels');
        await fill(filter, 'color-preview');
        await click('[role=treeitem][aria-label*="#color-preview —"]');
        ...
        const liveButton = await rect(button('Live button preview'));
        await page.mouse.move(liveButton.x + 15, liveButton.y + 15); await pause();
        const hoveredButton = await capture('button-override-hover');
        assert.deepEqual(await pixel(hoveredButton, liveButton.x + 12, liveButton.y + 12), [0, 255, 0], 'Button hover preserves edited background');
        await page.mouse.down(); await pause();
        const pressedButton = await capture('button-override-pressed');
        const pressedColor = await pixel(pressedButton, liveButton.x + 12, liveButton.y + 12);
        assert.ok(pressedColor[1] > 190 && pressedColor[1] > pressedColor[0] + 80, `Pressed button preserves edited hue: ${pressedColor}`);
