...
impl<M: AppModel> DevtoolsApp<M> {
    ...
    pub fn new(app: M) -> Self {
        ...
        Self {
            ...
            detached: WindowKey::new("__argui-devtools"),
            pending_detach: false,
            last_dock: DockMode::Bottom,
            error: None,
        }
    }
    ...
    pub fn set_dock_mode(&mut self, mode: DockMode) -> AppUpdate {
        if mode == DockMode::Detached {
            ...
            return AppUpdate::none().command(AppCommand::OpenWindow(WindowSpec::new(
                self.detached.clone(),
                WindowConfig {
                    title: "Argui DevTools".into(),
                    width: 900.0,
                    height: 650.0,
                    close_behavior: CloseBehavior::NotifyApp,
        ...
        if close {
            update.command(AppCommand::CloseWindow(self.detached.clone()))
        } else {
            update
        }
    }
    ...
    fn merge(mut update: AppUpdate, other: AppUpdate) -> AppUpdate {
        for invalidation in other.windows {
            update.invalidate(invalidation.window, invalidation.update);
        }
...
impl<M: AppModel> AppModel for DevtoolsApp<M> {
    ...
    fn update(&mut self, event: &AppEvent) -> AppUpdate {
        match event {
            AppEvent::WindowReady { window } if window == &self.detached && self.pending_detach => {
                ...
                AppUpdate::none()
                    .window(self.target.clone(), ViewUpdate::Rebuild)
                    .window(self.detached.clone(), ViewUpdate::Rebuild)
                    .command(AppCommand::FocusWindow(self.detached.clone()))
            }
            AppEvent::WindowFailed { window, error } if window == &self.detached => {
                self.failed(error)
            }
            AppEvent::Window {
                window,
                event: PlatformEvent::WindowCreationFailed(error),
            } if window == &self.detached => self.failed(error),
            AppEvent::Window {
                window,
                event: PlatformEvent::CloseRequested,
            } if window == &self.detached => self.close_tools(),
            AppEvent::Window {
                window,
                event: PlatformEvent::CloseRequested,
            ...
            AppEvent::Ui { window, event }
                if window == &self.target || window == &self.detached =>
            {
                if event
                    .target_key()
                    .is_some_and(|key| key.starts_with("__devtools"))
                {
                    ...
                    if !self.tools.get_mut().open
                        && (window == &self.detached || self.pending_detach)
                    {
                        update = Self::merge(update, self.close_tools());
                    }
                    update
                } else if self.app.event_router(window).is_none() {
                    self.app.update(&AppEvent::Ui {
                        window: window.clone(),
                        event: event.clone(),
                    })
                } else {
    ...
    fn animation_frame(&mut self, window: &WindowKey, frame: Frame) -> AppUpdate {
        let mut update = if window == &self.detached {
            AppUpdate::none()
        } else {
            self.app.animation_frame(window, frame)
        };
    ...
    fn inspector(&self, window: &WindowKey) -> Option<InspectorHandle> {
        if window == &self.target {
            Some(self.inspector())
        } else if window == &self.detached {
            None
        } else {
            self.app.inspector(window)
        }
    }
    fn tray(&self) -> Option<argui_platform::TrayConfig> {
        self.app.tray()
    }
    fn take_clipboard_request(&mut self, window: &WindowKey) -> Option<ClipboardRequest> {
        self.tools
            ...
            .or_else(|| self.app.take_clipboard_request(window))
    }
    fn take_scroll_request(&mut self, window: &WindowKey) -> Option<ScrollRequest> {
        self.tools
            .get_mut()
            .take_scroll_request()
            .or_else(|| self.app.take_scroll_request(window))
    }
    fn take_focus_request(&mut self, window: &WindowKey) -> Option<FocusRequest> {
        if window == self.tools_window() {
            self.tools
                .get_mut()
                .take_focus_request()
                .or_else(|| self.app.take_focus_request(window))
        } else {
            self.app.take_focus_request(window)
        }
    }
