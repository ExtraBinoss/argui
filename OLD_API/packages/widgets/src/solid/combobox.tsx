...
export function Combobox(props: ComboboxProps): JSX.Element {
  ...
  const edits = new InputEditController(displayValue())
  ...
  return <column width="fill" gap={6}>
    ...
    <focusScope key={props.id} role="combo_box" accessible_name={props.label}
      ...
      <rectangle width={width()} height={props.theme.inputHeight}
        ...
        <row width="fill" height="fill" min_width={0} gap={7} padding_left={props.theme.controlPadding}
          ...
          <container grow={1} min_width={0}>
            <textInput key={`${props.id}-input`} role="text_input" width="fill" height={24} clip={true}
              ...
              onBlur={() => setFocused(false)} onEdit={(payload) => edits.apply(payload, displayValue(), setQuery)} />
          </container>
    ...
    {expanded() ? <popupWindow key={`${props.id}-popup`} anchor={props.id} placement="bottom_start" width={width()}
          ...
          <column width="fill" max_height={260} scroll_y={true} gap={2} padding={4}>
            {filtered().length === 0
              ...
              : filtered().map(({ option, index }) => <column key={`${props.id}-option-row-${index}`} width="fill">
                ...
                <focusScope key={`${props.id}-option-${index}`} role="option" accessible_name={option.label}
