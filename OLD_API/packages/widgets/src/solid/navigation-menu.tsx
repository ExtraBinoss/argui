...
export function NavigationMenu(props: NavigationMenuProps): JSX.Element {
  const [uncontrolledOpen, setUncontrolledOpen] = createSignal<string | undefined>(props.defaultValue)
  const [uncontrolledActive, setUncontrolledActive] = createSignal(props.defaultValue
    ?? props.items.find((item) => !item.disabled)?.id ?? '')
  const [uncontrolledSelected, setUncontrolledSelected] = createSignal(props.defaultSelected ?? '')
  ...
  const active = () => {
    ...
    return props.items.some((item) => item.id === requested && !item.disabled)
  ...
  const renderEntry = (item: SurfaceNavigationItem) => {
    ...
    return <focusScope key={triggerId} role={hasPanel ? 'button' : 'link'} accessible_name={item.label}
  ...
  return <focusScope key={props.id} role="navigation" accessible_name={props.label ?? 'Primary navigation'}
...
function NavigationMenuPanel(props: NavigationMenuPanelProps): JSX.Element {
  const links = () => props.item.links ?? []
  const [activeId, setActiveId] = createSignal(links().find((link) => !link.disabled)?.id ?? '')
  ...
  return <popupWindow key={props.id} anchor={props.anchor} placement={props.placement} width={props.width}
    ...
    initial_focus="first" restore_focus={true} role="group" accessible_name={props.item.label}
    onDismiss={props.onClose}>
    <focusScope key={`${props.id}-content`} role="group" accessible_name={`${props.item.label} links`}
      ...
      <rectangle width="fill" background={props.theme.surface} border_color={props.theme.border} border_width={1}
        ...
        <column width="fill" gap={3} padding={8}>
          {links().map((link) => {
            ...
            return <focusScope key={linkKey} role="link" accessible_name={link.label}
