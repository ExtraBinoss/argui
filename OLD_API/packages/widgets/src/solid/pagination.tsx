...
export interface PaginationProps {
  ...
  /** List of page links and navigation actions. */ children: JSX.Element
}

/** Provides clamped page state to the composed pagination controls. */
export function Pagination(props: PaginationProps): JSX.Element {
  const [uncontrolled, setUncontrolled] = createSignal(props.defaultPage ?? 1)
  ...
  const content = <focusScope width="fill" role="navigation" accessible_name={props.label ?? 'Pagination'} focusable={false}>
    {props.children}
  </focusScope> as SolidJSX.Element
  return <PaginationContext.Provider value={context}>{content}</PaginationContext.Provider>
}

/** Props for the semantic list containing pagination entries. */
export interface PaginationContentProps { /** Page links and action buttons. */ children: JSX.Element }
...
export interface PaginationItemProps { /** One page link, navigation action, or decorative ellipsis. */ children: JSX.Element }
...
export interface PaginationLinkProps {
  ...
  /** Visible page number or custom native content. */ children?: JSX.Element
}
...
export function PaginationLink(props: PaginationLinkProps): JSX.Element {
  ...
  if (!pagination) throw new Error('PaginationLink must be rendered inside Pagination')
...
function PageButton(props: PageButtonProps): JSX.Element {
  ...
  return <focusScope key={props.targetId} role="link" accessible_name={props.label} current={props.active ? 'page' : undefined}
...
export function PaginationPrevious(props: PaginationPreviousProps = {}): JSX.Element {
  const pagination = useContext(PaginationContext)
  if (!pagination) throw new Error('PaginationPrevious must be rendered inside Pagination')
  const page = () => pagination.page() - 1
  const disabled = () => pagination.page() <= 1
  return <PageButton pagination={pagination} page={page()} targetId={`${pagination.id()}-previous`}
    label={props.label ?? 'Go to previous page'}
    disabled={disabled()} size="default">
    <text text="‹" color={disabled() ? pagination.theme().muted : pagination.theme().foreground} font_size={18} />
    <text text="Previous" color={disabled() ? pagination.theme().muted : pagination.theme().foreground} font_size={13} />
  </PageButton>
}
...
export function PaginationNext(props: PaginationNextProps = {}): JSX.Element {
  const pagination = useContext(PaginationContext)
  if (!pagination) throw new Error('PaginationNext must be rendered inside Pagination')
  const page = () => pagination.page() + 1
  ...
  return <PageButton pagination={pagination} page={page()} targetId={`${pagination.id()}-next`}
    label={props.label ?? 'Go to next page'}
    disabled={disabled()} size="default">
    <text text="Next" color={disabled() ? pagination.theme().muted : pagination.theme().foreground} font_size={13} />
    <text text="›" color={disabled() ? pagination.theme().muted : pagination.theme().foreground} font_size={18} />
  </PageButton>
}
...
export function PaginationEllipsis(_props: PaginationEllipsisProps = {}): JSX.Element {
  const pagination = useContext(PaginationContext)
  if (!pagination) throw new Error('PaginationEllipsis must be rendered inside Pagination')
