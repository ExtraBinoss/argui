...
import {
  ...
  type SurfaceMenuCheckbox, type SurfaceMenuController, type SurfaceMenuItem,
  type SurfaceMenuRadio, type SurfaceMenuStateProps, type SurfaceMenubarMenu,
...
export function Menubar(props: MenubarProps): JSX.Element {
  ...
  const [uncontrolledOpen, setUncontrolledOpen] = createSignal<string | undefined>(props.defaultOpenMenu)
  const [uncontrolledActive, setUncontrolledActive] = createSignal(props.defaultOpenMenu ?? first())
  ...
  const activeMenu = () => {
    const active = openMenu() ?? uncontrolledActive()
    return props.menus.some((menu) => menu.id === active && !menu.disabled) ? active : first()
  }
  ...
  return <focusScope key={props.id} role="menu_bar" accessible_name={props.label ?? 'Menu bar'}
    ...
    <rectangle width={props.width ?? 'fill'} height={40} background={props.theme.surfaceRaised}
      ...
      <row width="fill" height="fill" gap={2} padding={4} align_items="center">
        {props.menus.map((menu) => {
          ...
          return <focusScope key={triggerId} role="menu_item" accessible_name={menu.label}
