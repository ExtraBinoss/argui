...
export function ReactInputOTP(props: InputOTPProps): ReactElement {
  ...
  const [uncontrolled, setUncontrolled] = useState(normalizeInputOtpValue(props.defaultValue ?? '', length))
  ...
  const context: InputOTPContextValue = {
    value: () => normalizeInputOtpValue(props.value !== undefined ? props.value : uncontrolled, inputOtpLength(props.maxLength)),
    focused: () => focused,
    activeIndex: () => activeIndex,
    length: () => inputOtpLength(props.maxLength),
    cellSize: () => Number.isFinite(props.cellSize) && props.cellSize! > 0 ? props.cellSize! : 40,
    theme: () => props.theme,
    disabled: () => !!props.disabled,
    invalid: () => !!props.invalid,
  }
  return createElement(InputOTPContext.Provider, { value: context },
    <container width="fill" height={cellSize} position="relative">
      ...
      <container position="absolute" inset_left={0} inset_right={0} inset_top={0} inset_bottom={0}>
        <textInput nativeKey={props.id} role="text_input" width="fill" height="fill" clip={true}
...
export function ReactInputOTPGroup(props: InputOTPGroupProps): ReactElement {
...
export function ReactInputOTPSlot(props: InputOTPSlotProps): ReactElement | null {
  ...
  return <rectangle width={size} height={size} background={theme.surface}
    border_color={context.invalid() ? theme.destructive : active ? theme.accent : theme.border}
    ...
    <row width="fill" height="fill" align_items="center" justify_content="center">
      {character ? <text text={character} color={theme.foreground} font_size={theme.controlFontSize}
...
export function ReactInputOTPSeparator(props: InputOTPSeparatorProps): ReactElement {
  const context = useContext(InputOTPContext)
