...
import { ReactPopover as Popover } from './popover'
...
export function ReactCalendar(props: ReactCalendarProps): ReactElement {
  ...
  return <column nativeKey={props.id} width={gridWidth} gap={8} role="group" accessible_name={props.label ?? 'Choose date'}>
    <row width="fill" height={34} align_items="center">
      ...
      <container grow={1} min_width={0}>
        <row width="fill" height={34} gap={2} align_items="center" justify_content="center">
          <focusScope key={monthPickerId} nativeKey={monthPickerId} role="button" accessible_name={`Choose month, ${monthName}`}
          ...
          <focusScope key={yearPickerId} nativeKey={yearPickerId} role="button" accessible_name={`Choose year, ${displayedMonth.getFullYear()}`}
    ...
    <Popover id={monthPickerId} label="Months" theme={props.theme} trigger={false} open={openPicker === 'month'}
      ...
      <focusScope role="list_box" accessible_name="Months" focusable={false}>
          <column width="fill" max_height={220} scroll_y={true} gap={2}>
            {Array.from({ length: 12 }, (_, month) => <focusScope key={`${monthPickerId}-${month}`} nativeKey={`${monthPickerId}-${month}`} role="option"
    ...
    <Popover id={yearPickerId} label="Years" theme={props.theme} trigger={false} open={openPicker === 'year'}
      ...
      virtualItems={{ count: years().length, itemHeight: 32, height: 220,
        ...
        renderItem: (index) => {
          ...
          return <focusScope key={`${yearPickerId}-${year}`} nativeKey={`${yearPickerId}-${year}`}
            role="option" accessible_name={`${year}`} selected={year === displayedMonth.getFullYear()}
            keyboard_activation="enter_or_space" onClick={() => chooseYear(year)}>
            <touchArea mouse_cursor="pointer">
    ...
    <focusScope nativeKey={gridId} role="grid" accessible_name={props.label ?? 'Choose date'}
      ...
      <column width="fill" gap={cellGap}>
        <row width="fill" gap={cellGap} role="row">
          {days.map((day) => <column key={`${props.id}-weekday-${day.full}`} nativeKey={`${props.id}-weekday-${day.full}`} width={cellWidth} height={24}
        ...
        {Array.from({ length: 6 }, (_, weekIndex) => <row key={`${props.id}-week-${weekIndex}`} nativeKey={`${props.id}-week-${weekIndex}`} width="fill" gap={cellGap} role="row">
          {cells.slice(weekIndex * 7, weekIndex * 7 + 7).map((cell) => {
            if (!cell.inMonth && props.showOutsideDays === false) {
              return <rectangle key={`${props.id}-blank-${cell.key}`} nativeKey={`${props.id}-blank-${cell.key}`} width={cellWidth} height={36} accessible_hidden={true} />
            }
