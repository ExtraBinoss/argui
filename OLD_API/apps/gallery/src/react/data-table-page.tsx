...
export function DataTablePage(props: { theme: Palette }): ReactElement {
  const [query, setQuery] = useState('')
  const [status, setStatus] = useState<DataTableStatus>('All')
  const [sort, setSort] = useState<TableSort | null>({ columnId: 'reference', direction: 'ascending' })
  const [page, setPage] = useState(1)
  const [selectedIds, setSelectedIds] = useState<readonly string[]>([])
  const view = useMemo(() => dataTableView(query, status, sort, page, pageSize), [query, status, sort, page])
  ...
  return <column width="fill" gap={14}>
    ...
    <row wrap={true} gap={6} align_items="center">
      ...
      {statuses.map((option) => <Button key={option} id={`data-table-filter-${option.toLowerCase()}`} label={option}
    ...
    <Pagination id="data-table-pagination" label="Order pages" theme={props.theme}
      ...
      <PaginationContent>
        ...
        {pageLinks.map((number) => <PaginationItem key={number}><PaginationLink page={number} /></PaginationItem>)}
