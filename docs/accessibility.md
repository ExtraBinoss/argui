...
per interactive element. Pan waits for an eight-pixel threshold; pinch and
rotation can run simultaneously for a contact pair. Velocity uses a bounded
100 ms history, and cancellation produces explicit terminal events. The primary
touch contact also drags scroll containers directly while all contacts remain
