...
- **Ordre impératif : terminer d'abord le chantier thème du [plan global](argui-crate-theme-decisions.md#contrat-du-thème-à-rendre-réel), puis commencer les widgets shadcn.** Cette première phase inclut le raccordement réel d'`argui-theme` aux apps Rust pur et TSX, les adaptateurs Solid/React, la migration complète de la gallery et de ses cinq widgets existants, et le retrait d'`argui-reactive` après remplacement de ses usages du thème. Vérifier ses critères avant de donner un premier lot shadcn aux Luna. Les consolidations shader/mobile et les autres options CLI du plan global ne bloquent pas ce chantier.
...
1. **Coordinateur : Sol 6, effort `xhigh`.** Lire `AGENTS.md`, le présent plan et le plan global. **Phase 1 obligatoire : implémenter et vérifier le thème commun de bout en bout** dans une app Rust pur, les adaptateurs TSX Solid/React, les cinq widgets actuels et la gallery Solid/React, puis retirer `argui-reactive` après migration. Les tokens, variantes, overrides, changements atomiques et préférences système doivent être utilisables en direct. Aucun sous-agent n'entame un nouveau widget shadcn tant que cette phase n'est pas terminée. Le coordinateur peut déléguer des sous-tâches du thème avec des fichiers propriétaires séparés.
2. **Phase 2, parité shadcn :** ouvrir les sources shadcn committées, cartographier les primitives Argui manquantes, les dépendances entre widgets et le plan du registre. Maintenir la checklist et les décisions de compatibilité. **Implémentation : Luna MAX** (`gpt-6-luna`, effort `max`) sous coordination du Sol. Donner à chaque Luna un lot borné de **trois composants à la suite**, avec fichiers propriétaires explicites et les références TSX/exemples correspondantes. Le lot comprend Solid, React, styles/tokens, démos et métadonnées de registre de ses trois composants. Dès qu'un lot est intégré, lui confier trois autres composants. Le Sol prend les primitives partagées, conflits de fichiers et choix d'API ; plusieurs Luna n'éditent jamais simultanément `registry.json`, `gallery-pages.ts` ou les mêmes index. Il peut séquencer leur intégration.
